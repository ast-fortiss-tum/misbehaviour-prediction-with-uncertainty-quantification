...
def compute_tp_and_fn(data_df_anomalous, losses_on_anomalous, threshold, seconds_to_anticipate,
    ...
    ''' 
        ...
        '''
    #fixed fps for icse20 benchmark and dynamic fps for other benchmarks
    if cond=='icse20':
        number_frames_anomalous = pd.Series.max(data_df_anomalous['frameId'])
        fps_anomalous = 35  # only for icse20 configurations
    else:
        number_frames_anomalous = pd.Series.max(data_df_anomalous['frameId'])
...
def compute_fp_and_tn(cfg,data_df_nominal, aggregation_method, condition):
    ...
    #fixed fps for icse20 benchmark and dynamic fps for other benchmarks
    if condition=='icse20':
        fps_nominal = 35  # only for icse20 configurations
    else:
...
def evaluate_failure_prediction(cfg, simulation_name, aggregation_method, condition, model,method):
    ...
    for seconds in range(1, 4):
        ...
        if not os.path.exists('results/icse35_dynamic/t99999/'+ model+'.csv'):
            #Change the path below to change the results output folder
            with open('results/icse35_dynamic/t99999/'+ model+'.csv', mode='w',
            ...
            #Change the path below to change the results output folder
            with open('results/icse35_dynamic/t99999/'+ model+'.csv', mode='a',
                      newline='') as result_file:
                writer = csv.writer(result_file,
                                    delimiter=',',
                                    quotechar='"',
