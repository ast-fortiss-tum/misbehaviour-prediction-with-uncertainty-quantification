...
def evaluate_failure_prediction(cfg, simulation_name, aggregation_method, condition, model):
    ...
    path = os.path.join(cfg.TESTING_DATA_DIR, 'DAVE2-Track1-Normal','driving_log_cut.csv')
    ...
    #Cut the first 100 Frames and use a whole lap (simulation file contains 2 laps)
    ...
    #cut the first 100 Frames and use a whole lap (simulation file contains 2 laps)
    new_losses = new_losses[100:len(data_df_anomalous)+100]
    ...
    for seconds in range(1, 4):
        ...
        if not os.path.exists('results/t99_cut/'+ model+'.csv'):
            #change the path to choose the output folder
            with open('results/t99_cut/'+ model+'.csv', mode='w',
            ...
            #change the path to choose the output folder
            with open('results/t99_cut/'+ model+'.csv', mode='a',
                      newline='') as result_file:
                writer = csv.writer(result_file,
                                    delimiter=',',
                                    quotechar='"',
