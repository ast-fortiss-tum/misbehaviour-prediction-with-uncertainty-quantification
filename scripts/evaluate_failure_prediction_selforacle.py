...
import utils
from scripts.evaluate_failure_prediction_heatmaps_scores_15fps import compute_fp_and_tn, compute_tp_and_fn
...
def evaluate_failure_prediction(cfg, simulation_name, aggregation_method, condition):
    ...
    for seconds in range(1, 4):
        ...
        if not os.path.exists('results/dynamic/t99999/'+'SelfOracle.csv'):
            with open('results/dynamic/t99999/'+'SelfOracle.csv', mode='w',
            ...
            with open('results/dynamic/t99999/'+'SelfOracle.csv', mode='a',
                      newline='') as result_file:
                writer = csv.writer(result_file,
                                    delimiter=',',
                                    quotechar='"',
