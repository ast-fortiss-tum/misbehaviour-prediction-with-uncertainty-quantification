...
def compute_tp_and_fn(data_df_anomalous, losses_on_anomalous, threshold, seconds_to_anticipate,
    ...
    if True:
        ...
        fps_anomalous = 35  # all configurations fixed fps
    else:#code below for dynamic fps
        number_frames_anomalous = pd.Series.max(data_df_anomalous['frameId'])
...
def compute_fp_and_tn(cfg,data_df_nominal, aggregation_method, condition='icse20'):
    # when conditions == nominal I count only FP and TN

    if True:
        fps_nominal = 35  #Fixed fps across all conditions
    else:#code below for dynamic fps
...
def evaluate_failure_prediction(cfg, simulation_name, aggregation_method, condition, model,method):
    ...
    path = os.path.join(cfg.TESTING_DATA_DIR, 'icse20/DAVE2-Track1-Normal','driving_log.csv')
    data_df_nominal = pd.read_csv(path)


    path = os.path.join('plots/uncertainty/MC', 'icse20/DAVE2-Track1-Normal', model+'.csv')
    ...
    for seconds in range(1, 4):
        ...
        if not os.path.exists('results/final_99999/'+ model+'.csv'):
            #Change the path below to change the results output folder
            with open('results/final_99999/'+ model+'.csv', mode='w',
            ...
            #Change the path below to change the results output folder
            with open('results/final_99999/'+ model+'.csv', mode='a',
                      newline='') as result_file:
                writer = csv.writer(result_file,
                                    delimiter=',',
                                    quotechar='"',
